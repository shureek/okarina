...
3. **Запуск**:
   ...
   ```
4. **Результат**: в `notes/` появятся файлы вида `0la.svg`, `do.svg`, `sol+.svg`, `2do+.svg` и т.д. Каждый файл содержит рисунок аппликатуры с подписью ноты. Префикс `0` — три нижние ступени (ля, ля♯, си) ниже центральной «до»; префикс `2` — верхняя октава на инструменте. Старые SVG сохранены в папке `notes-old/` для справки.
...
4. **Поддерживаемые обозначения**:
   ...
   - нижняя октава на инструменте: `0ля`, `ля0`, `0la`, `la0` и т.п.
